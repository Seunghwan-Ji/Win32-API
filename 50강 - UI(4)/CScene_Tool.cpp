...
void CScene_Tool::Enter()
{
	...
	// 부모 UI 오브젝트 생성.
	CUI* pParentUI = new CUI(false);
	pParentUI->SetName(L"ParentUI");
	pParentUI->SetScale(Vec2(500.f, 300.f));
	pParentUI->SetPos(Vec2(vResolution.x - pParentUI->GetScale().x, 0.f));
	...
	pParentUI->AddChild(pChildUI); // 부모 UI 에 자식 UI 추가.

	AddObject(pParentUI, GROUP_TYPE::UI); // 부모 UI 를 추가 함으로써 자식 UI 가 같이 추가됨.
