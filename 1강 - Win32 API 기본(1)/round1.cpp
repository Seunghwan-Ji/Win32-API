...
// 프로그램 실행 시 호출되는 메인 함수.
// _In_, _In_opt_: 변수의 의미를 적는 키워드, SAL 이라는 주석 언어이다.
// _In_: 함수에 데이터가 입력된다는 뜻.
// _In_opt_: 딱히 필요없는 부가적인 데이터라는 뜻.
int APIENTRY wWinMain(_In_     HINSTANCE hInstance // 실행된 프로세스의 시작 주소.
    , _In_opt_ HINSTANCE hPrevInstance
    , _In_     LPWSTR    lpCmdLine // 문자열 주소 타입.
    , _In_     int       nCmdShow)
{
    // 참조되지 않는 변수들이라고 알려주는 주석 개념
    UNREFERENCED_PARAMETER(hPrevInstance);
    UNREFERENCED_PARAMETER(lpCmdLine);
    // 사실상 의미 없는 코드이므로 무시한다.
    ...
    // 윈도우 생성.
    // 윈도우: 사용자 인터페이스, 사용자와 컴퓨터간 상호작용.
    // 윈도우와 프로세스는 전혀 다른 개념이다.
    // 애플리케이션 초기화를 수행합니다:
